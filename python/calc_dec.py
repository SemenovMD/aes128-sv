#!/usr/bin/env python3
"""
AES-128 Decryption Step-by-Step with NIST Validated Test Vectors
"""

inv_sbox = [
    
]

def bytes_to_matrix(data):
    matrix = [[0] * 4 for _ in range(4)]
    for i in range(4):
        for j in range(4):
            matrix[j][i] = data[i * 4 + j]
    return matrix

def matrix_to_bytes(matrix):
    data = bytearray(16)
    for i in range(4):
        for j in range(4):
            data[i * 4 + j] = matrix[j][i]
    return bytes(data)

def print_state(state, title):
    print(f"{title}:")
    for i in range(4):
        row = [f"{state[j][i]:02X}" for j in range(4)]
        print("  " + " ".join(row))

def inv_shift_rows(state):
    new_state = [row[:] for row in state]
    new_state[1] = [state[1][3], state[1][0], state[1][1], state[1][2]]
    new_state[2] = [state[2][2], state[2][3], state[2][0], state[2][1]]
    new_state[3] = [state[3][1], state[3][2], state[3][3], state[3][0]]
    return new_state

def inv_sub_bytes(state):
    return [[inv_sbox[b] for b in row] for row in state]

def add_round_key(state, round_key):
    key_matrix = bytes_to_matrix(round_key)
    return [[state[i][j] ^ key_matrix[i][j] for j in range(4)] for i in range(4)]

def gmul(a, b):
    p = 0
    for _ in range(8):
        if b & 1:
            p ^= a
        carry = a & 0x80
        a <<= 1
        if carry:
            a ^= 0x1B
        b >>= 1
    return p & 0xFF

def inv_mix_columns(state):
    new_state = [[0]*4 for _ in range(4)]
    for col in range(4):
        column = [state[row][col] for row in range(4)]
        new_state[0][col] = gmul(0x0E, column[0]) ^ gmul(0x0B, column[1]) ^ gmul(0x0D, column[2]) ^ gmul(0x09, column[3])
        new_state[1][col] = gmul(0x09, column[0]) ^ gmul(0x0E, column[1]) ^ gmul(0x0B, column[2]) ^ gmul(0x0D, column[3])
        new_state[2][col] = gmul(0x0D, column[0]) ^ gmul(0x09, column[1]) ^ gmul(0x0E, column[2]) ^ gmul(0x0B, column[3])
        new_state[3][col] = gmul(0x0B, column[0]) ^ gmul(0x0D, column[1]) ^ gmul(0x09, column[2]) ^ gmul(0x0E, column[3])
    return new_state

def key_expansion(key):
    rcon = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36]
    
    sbox = [
        
    ]
    
    key_bytes = list(key)
    w = [0] * 44
    
    for i in range(4):
        w[i] = key_bytes[4*i:4*i+4]
    
    for i in range(4, 44):
        temp = w[i-1][:]
        
        if i % 4 == 0:
            temp = temp[1:] + temp[:1]
            temp = [sbox[b] for b in temp]
            temp[0] ^= rcon[i//4 - 1]
        
        w[i] = [w[i-4][j] ^ temp[j] for j in range(4)]
    
    keys = []
    for i in range(11):
        key_round = []
        for j in range(4):
            key_round.extend(w[4*i + j])
        keys.append(bytes(key_round))
    
    return keys

def aes128_decrypt_step_by_step(ciphertext, round_keys):
    print("=" * 70)
    print("AES-128 DECRYPTION STEP BY STEP")
    print("=" * 70)
    
    state = bytes_to_matrix(ciphertext)
    print_state(state, "Initial Ciphertext")
    
    print(f"\n{'='*50}")
    print("INITIAL ROUND (10) - AddRoundKey only")
    print(f"{'='*50}")
    
    state = add_round_key(state, round_keys[10])
    print_state(state, "After AddRoundKey(K10)")
    
    for round_num in range(9, 0, -1):
        print(f"\n{'='*50}")
        print(f"ROUND {round_num}")
        print(f"{'='*50}")
        
        state = inv_shift_rows(state)
        print_state(state, "After InvShiftRows")
        
        state = inv_sub_bytes(state)
        print_state(state, "After InvSubBytes")
        
        state = add_round_key(state, round_keys[round_num])
        print_state(state, f"After AddRoundKey(K{round_num})")
        
        state = inv_mix_columns(state)
        print_state(state, "After InvMixColumns")
    
    print(f"\n{'='*50}")
    print("FINAL ROUND (0) - No InvMixColumns")
    print(f"{'='*50}")
    
    state = inv_shift_rows(state)
    print_state(state, "After InvShiftRows")
    
    state = inv_sub_bytes(state)
    print_state(state, "After InvSubBytes")
    
    state = add_round_key(state, round_keys[0])
    print_state(state, "After AddRoundKey(K0)")
    
    plaintext = matrix_to_bytes(state)
    return plaintext

def test_nist_vector_1():
    print("=== NIST TEST VECTOR 1 ===")
    
    key = bytes.fromhex("2b7e151628aed2a6abf7158809cf4f3c")
    ciphertext = bytes.fromhex("3ad77bb40d7a3660a89ecaf32466ef97")
    expected_plaintext = bytes.fromhex("6bc1bee22e409f96e93d7e117393172a")
    
    print(f"Key:        {key.hex()}")
    print(f"Ciphertext: {ciphertext.hex()}")
    print(f"Expected:   {expected_plaintext.hex()}")
    
    round_keys = key_expansion(key)
    
    print(f"\n=== ROUND KEYS ===")
    for i, k in enumerate(round_keys):
        print(f"K{i:2d}: {k.hex()}")
    
    plaintext = aes128_decrypt_step_by_step(ciphertext, round_keys)
    
    print(f"\n{'='*70}")
    print("FINAL RESULT")
    print(f"{'='*70}")
    print(f"Decrypted:  {plaintext.hex()}")
    print(f"Expected:   {expected_plaintext.hex()}")
    print(f"MATCH:      {plaintext == expected_plaintext}")

def test_nist_vector_2():
    print("\n" + "="*70)
    print("=== NIST TEST VECTOR 2 ===")
    
    key = bytes.fromhex("2b7e151628aed2a6abf7158809cf4f3c")
    ciphertext = bytes.fromhex("f5d3d58503b9699de785895a96fdbaaf")
    expected_plaintext = bytes.fromhex("ae2d8a571e03ac9c9eb76fac45af8e51")
    
    print(f"Key:        {key.hex()}")
    print(f"Ciphertext: {ciphertext.hex()}")
    print(f"Expected:   {expected_plaintext.hex()}")
    
    round_keys = key_expansion(key)
    
    plaintext = aes128_decrypt_step_by_step(ciphertext, round_keys)
    
    print(f"\n{'='*70}")
    print("FINAL RESULT")
    print(f"{'='*70}")
    print(f"Decrypted:  {plaintext.hex()}")
    print(f"Expected:   {expected_plaintext.hex()}")
    print(f"MATCH:      {plaintext == expected_plaintext}")

if __name__ == "__main__":
    test_nist_vector_1()
    test_nist_vector_2()