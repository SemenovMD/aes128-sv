#!/usr/bin/env python3
"""
AES-128 Encryption Step-by-Step with NIST Validated Test Vectors
"""
...
def mix_columns(state):
    ...
    for col in range(4):
        ...
        new_state[0][col] = gmul(0x02, column[0]) ^ gmul(0x03, column[1]) ^ column[2] ^ column[3]
        new_state[1][col] = column[0] ^ gmul(0x02, column[1]) ^ gmul(0x03, column[2]) ^ column[3]
        new_state[2][col] = column[0] ^ column[1] ^ gmul(0x02, column[2]) ^ gmul(0x03, column[3])
        new_state[3][col] = gmul(0x03, column[0]) ^ column[1] ^ column[2] ^ gmul(0x02, column[3])
    return new_state
...
def key_expansion(key):
    ...
    for i in range(11):
        key_round = []
        for j in range(4):
            key_round.extend(w[4*i + j])
        keys.append(bytes(key_round))
    
    return keys

def aes128_encrypt_step_by_step(plaintext, round_keys):
    print("=" * 70)
    print("AES-128 ENCRYPTION STEP BY STEP")
    print("=" * 70)
    ...
    print(f"\n{'='*50}")
    print("INITIAL ROUND (0) - AddRoundKey only")
    print(f"{'='*50}")
    
    state = add_round_key(state, round_keys[0])
    print_state(state, "After AddRoundKey(K0)")
    ...
    for round_num in range(1, 10):
        ...
        print_state(state, f"After AddRoundKey(K{round_num})")
    
    print(f"\n{'='*50}")
    print("FINAL ROUND (10) - No MixColumns")
    print(f"{'='*50}")
...
def test_nist_vector_1():
    ...
    ciphertext = aes128_encrypt_step_by_step(plaintext, round_keys)
...
def test_nist_vector_2():
    ...
    expected_ciphertext = bytes.fromhex("f5d3d58503b9699de785895a96fdbaaf")
