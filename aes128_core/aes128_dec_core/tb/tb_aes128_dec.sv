...
module tb_aes128_dec;
    ...
    // Static key (the same as in encryption)
    localparam logic [127:0] STATIC_KEY = 128'h2B7E151628AED2A6ABF7158809CF4F3C;
    
    // Test vectors for DECRYPTION: ciphertext input and expected plaintext output
    ...
    initial begin
        // For DECRYPTOR: ciphertext as input, plaintext as expected output
    ...
    initial begin
        key[0]  = 128'hD014F9A8C9EE2589E13F0CC8B6630CA6;
        key[1]  = 128'hAC7766F319FADC2128D12941575C006E; 
        key[2]  = 128'hEAD27321B58DBAD2312BF5607F8D292F;
        key[3]  = 128'h4E54F70E5F5FC9F384A64FB24EA6DC4F;
        key[4]  = 128'h6D88A37A110B3EFDDBF98641CA0093FD;
        key[5]  = 128'hD4D1C6F87C839D87CAF2B8BC11F915BC;
        key[6]  = 128'hEF44A541A8525B7FB671253BDB0BAD00;
        key[7]  = 128'h3D80477D4716FE3E1E237E446D7A883B;
        key[8]  = 128'hF2C295F27A96B9435935807A7359F67F;
        key[9]  = 128'hA0FAFE1788542CB123A339392A6C7605;
        key[10] = 128'h2B7E151628AED2A6ABF7158809CF4F3C;
    end
    
    initial begin
        wait(aresetn === 1'b1);
        $display("\nStarting AES-128 DECRYPTION Testbench with Static Key");
    ...
    task automatic send_all_tests_burst();
        ...
        $display("Sending %0d AES DECRYPTION tests in BURST mode...", MAX_TESTS);
        
        @(posedge aclk);
        s_axis_tvalid = 1'b1;

        while (i < MAX_TESTS) begin
            s_axis_tdata = ciphertext[i];
        ...
        s_axis_tvalid = 1'b0;
        $display("All AES decryption tests sent in burst mode");
    endtask
    
    task automatic receive_all_results();
        int current_test_id = 0;
        
        $display("Receiving AES DECRYPTION results...");
        ...
        while (tests_received < MAX_TESTS) begin
            ...
            if (current_test_id < MAX_TESTS) begin
                if (m_axis_tdata === plaintext[current_test_id]) begin
                    if (tests_received < 5) begin
                        $display("PASS Test %0d", current_test_id+1);
                        $display("   Ciphertext: %032h", ciphertext[current_test_id]);
                        $display("   Plaintext:  %032h", m_axis_tdata);
                    end
                end else begin
                    $display("FAIL Test %0d", current_test_id+1);
                    $display("   Ciphertext: %032h", ciphertext[current_test_id]);
                    $display("   Key:        %032h", STATIC_KEY);
                    $display("   Expected:   %032h", plaintext[current_test_id]);
        ...
        $display("All AES decryption results received");
    endtask
    ...
    function automatic void print_test_summary();
        ...
        real throughput;
        real data_rate;
        real speed_per_decryption;
        
        $display("\n====================================================================");
        $display("                    AES-128 DECRYPTION TEST SUMMARY");
        $display("====================================================================");
        $display("Static Key:     %032h", STATIC_KEY);
        ...
        if (test_count > 0) begin
            ...
            speed_per_decryption = total_time_ns / real'(test_count);
            ...
            $display("  Throughput: %0.2f decryptions/sec", throughput);
            $display("  Speed:      %0.2f ns/decryption", speed_per_decryption);
        ...
        if (error_count == 0 && tests_sent == tests_received) begin
            $display("\nALL AES DECRYPTION TESTS PASSED SUCCESSFULLY!");
        end else begin
            $display("\nAES DECRYPTION TEST FAILURES DETECTED!");
