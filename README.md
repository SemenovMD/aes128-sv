# High-Throughput AES-128 Core

Модуль представляет собой высокопроизводительную реализацию `AES-128 алгоритма` на `SystemVerilog`, обеспечивающую максимальную пропускную способность до `1 операции за такт`. Архитектура основана на кластере параллельных ядер с интегрированным `AXI-Stream` интерфейсом, оптимизированная для развертывания на FPGA платформах `Xilinx`.
